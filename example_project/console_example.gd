...
func _spawn_entity(pos_x: int, pos_y: int, ttl: float) -> String:
	...
	var custom_interaction: Interaction = Interaction.new()
	var custom_edit: Interaction = Interaction.new()
...
func _trigger_spawn_entity(x: int, y: int, ttl: float) -> void:
	var target_position:Vector2 = Vector2(x, y)
	var new_node: EntityExample = EntityExample.new(target_position, ttl)
	add_child(new_node)
...
func _count_up(amount: String) -> String:
	...
	var parsed_amount: int = int(amount)
	var old_counter: int = counter
...
func _open_custom_console() -> void:
	Console.update_console_settings(func(settings: ConsoleSettings) -> void:
		settings.custom_template = custom_console_template
	)
...
func _open_default_console() -> void:
	Console.update_console_settings(func(settings: ConsoleSettings) -> void:
...
func _handle_unknown_interaction(interaction: Interaction) -> void:
	match interaction.get_type():
		"spawn_entity":
			## In theory this would be the entity we want to spawn, this is added just for demo purpose
			var _entity_name: String = interaction.get_data()
			var x: int = interaction.get_additional_data().get_or_add("x", 0)
			var y: int = interaction.get_additional_data().get_or_add("y", 0)
			var ttl: int = interaction.get_additional_data().get_or_add("ttl", 0)
